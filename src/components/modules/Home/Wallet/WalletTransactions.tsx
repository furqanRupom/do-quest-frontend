    ...
    <Card className="bg-card border-border backdrop-blur-xl overflow-hidden">
      ...
      <div className="p-6 border-b border-border flex justify-between items-center bg-muted/50">
        ...
        <Button
          variant="ghost"
          size="sm"
          className="text-muted-foreground hover:text-foreground flex items-center gap-2 text-xs font-semibold uppercase tracking-wider"
        >
          <ListFilter size={14} />
          Filter
        </Button>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border hover:bg-transparent">
              <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wider px-8 py-5">
                Date
              </TableHead>
              <TableHead className="text-muted-foreground text-xs font-semibold uppercase tracking-wider px-8 py-5">
                Description
          ...
          <TableBody className="divide-y divide-border">
            {transactions.map((tx, index) => {
              ...
              const amountDisplay = `${isCredit ? "+" : "-"}$${Math.abs(tx.amount).toLocaleString(
                "en-US",
              ...
              return (
                ...
                >
                  <TableCell className="px-8 py-6 text-muted-foreground text-sm">
                    {/* In real app, format tx.createdAt here */}
      ...
      <div className="p-6 bg-muted/50 border-t border-border flex justify-center">
        <Button
          variant="ghost"
          className="text-primary font-bold hover:text-primary/80 hover:bg-transparent flex items-center gap-2 text-sm"
        >
