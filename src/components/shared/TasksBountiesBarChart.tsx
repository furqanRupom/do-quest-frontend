...
interface TasksBountiesBarChartProps {
    data : BarChartData[]
}

const TasksBountiesBarChart = ({data}: TasksBountiesBarChartProps) => {

    if(!data || !Array.isArray(data)){
        return (
            <Card className="col-span-4">
                <CardHeader>
                    <CardTitle>Tasks and Bounties Trends</CardTitle>
                    <CardDescription>Monthly Tasks and Bounties Statistics</CardDescription>
                </CardHeader>
    ...
    if(!formattedData.length || formattedData.every(item => item.appointments === 0)){
        return (
          <Card className="col-span-4">
            ...
            <CardContent className="flex items-center justify-center h-75">
              <p className="text-sm text-muted-foreground">
                No tasks and bounties data available.
              </p>
            </CardContent>
          </Card>
        );
...
export default TasksBountiesBarChart
